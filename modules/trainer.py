...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks, align_ids, align_masks) in enumerate(self.train_dataloader):
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, align_ids, align_masks) in enumerate(self.val_dataloader):
                images, reports_ids, reports_masks, align_ids, align_masks = images.to(self.device), reports_ids.to(self.device), \
                                                     reports_masks.to(self.device), align_ids.to(self.device), align_masks.to(self.device)
